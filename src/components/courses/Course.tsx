    ...
    render() {
        ...
        if (!this.props.courses || this.props.courses.empty() || !this.props.courseId) {
            ...
            if (!courseData) {
                ...
                //See if it specfied
                if (this.props.lessonNumber) lessonIndex = parseInt(this.props.lessonNumber)
                ...
                const lessonList = courseData.lessons.map((lesson, index) => {
                    return {
                        text: index + ": " + lesson.name, value: index,
                    }
                })
                ...
                return (<div>
                        ...
                        {/*Add a progress bar*/}
                        <br/>
                        {courseData.lessons.length > 1 && <Grid columns='equal'>
                            <Grid.Column textAlign='right' width={2}>{prevButton}</Grid.Column>
                            <Grid.Column textAlign='center' size='tiny' as={Progress}
                                         percent={percent}>{courseData.lessons[lessonIndex].name}</Grid.Column>
